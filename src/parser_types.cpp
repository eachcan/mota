...
std::shared_ptr<ast::Type> Parser::primaryType() {
    // 处理容器类型（允许嵌套）
...
std::shared_ptr<ast::Type> Parser::containerType() {
    
    if (consume(lexer::TokenType::Optional)) {
        // 解析可选类型
        auto valueType = primaryType();
        return makeNode<ast::ContainerType>(ast::ContainerType::Kind::Optional, valueType);
    } else if (consume(lexer::TokenType::Repeated)) {
        // 解析数组类型
        auto elementType = primaryType();
        return makeNode<ast::ContainerType>(ast::ContainerType::Kind::Array, elementType);
        ...
        auto containerType = makeNode<ast::ContainerType>(ast::ContainerType::Kind::Map, valueType);
        
        // 设置键类型为 string
        auto keyType = makeNode<ast::NamedType>("string");
        containerType->keyType = keyType;
